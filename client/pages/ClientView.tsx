    ...
    return (
      <div className="container mx-auto p-6">
        <div className="text-center">
          ...
          <p className="text-gray-600 mt-2">The requested account could not be found.</p>
        </div>
      </div>
    );
  }
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-gray-600">{account.industry} • {account.type}</p>
          </div>
        </div>
            ...
            <Card>
              ...
              <CardContent className="space-y-3">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-sm font-medium">{account.location || 'Not specified'}</span>
                </div>
                <div className="flex items-center space-x-2">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-sm font-medium">{account.employees || 'Not specified'}</span>
                </div>
              </CardContent>
            </Card>
              ...
              <div className="space-y-4">
                {accountContacts.map((contact) => (
                  <div key={contact.id} className="flex items-center justify-between p-4 border rounded-lg">
                    ...
                    <Badge variant={contact.status === 'Active Deal' ? 'default' : 'secondary'}>
                      {contact.status}
                    </Badge>
                  </div>
                ))}
                {accountContacts.length === 0 && (
                  <p className="text-center text-gray-500 py-8">No contacts found for this account</p>
                )}
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent>
              <div className="space-y-4">
                {accountDeals.map((deal) => (
                  <div key={deal.id} className="border rounded-lg p-4">
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <Badge variant={deal.stage === 'Order Won' ? 'default' : 'secondary'}>
                        {deal.stage}
                      </Badge>
                    </div>
                ...
                {accountDeals.length === 0 && (
                  <p className="text-center text-gray-500 py-8">No opportunities found for this account</p>
                )}
              </div>
            </CardContent>
          </Card>
              ...
              <div className="space-y-4">
                {interactions.map((interaction) => (
                  <div key={interaction.id} className="border rounded-lg p-4">
                    ...
                    <div className="flex items-center justify-between text-sm">
                      <span className="text-gray-500">Contact: {interaction.contact}</span>
                      <span className="text-gray-500">Outcome: {interaction.outcome}</span>
                    </div>
                  </div>
                ))}
              </div>
